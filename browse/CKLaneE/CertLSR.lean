import CKLaneE.CertLS3

/-!
# Lane E: relative-floor log-sum certificate `lsr`

For archived same-side boxes the entropy floor is relative, `E ≥ t0·C0` with `C0 = (H a + H b)/2`.
Then the law's deficit satisfies `s ≤ S := (1 - t0)·C0`, and `S ∈ [CLo(1-t0), CHi(1-t0)]`,
`Δ + S = H(m) - t0·C0 ≤ H(mHi) - t0·CLo`.  The certificate is `LS3` (tight drop bounds, concave
endpoints) evaluated on these relative deficit bounds; the box's `E1`/`E2` fields are not used.
-/

set_option autoImplicit false
set_option relaxedAutoImplicit false

namespace CKLaneE.LSR
...
def sLoR : ℚ := CLo B * (1 - t0)
def sHiR : ℚ := CHi B * (1 - t0)
def IHiR : ℚ := Hhi (mHi B) - t0 * CLo B
...
def norm2 (v0S v0I v1S v1I : ℚ) : Bool :=
  decide (Kx B * (pbar v0S v0I - 4) ≤ Wlo B + betaLo B * sLoR B t0 ∧
    Kx B * (pbar v1S v1I - 4) ≤ Wlo B + betaLo B * sHiR B t0)

def mean2 (v0S v0I v1S v1I : ℚ) : Bool :=
  decide (B.r2 < 1) && ptOk B.r2 &&
    decide (Dm B * pbar v0S v0I ≤ jLo B + betaLo B * (dLo B * dLo B) * sLoR B t0 ∧
      Dm B * pbar v1S v1I ≤ jLo B + betaLo B * (dLo B * dLo B) * sHiR B t0)
...
/-- Soundness of an `lsr` leaf (relative entropy floor `t0·C0`). -/
  ...
  have hGS : P (Δ + S) - P S ≤ interiorCost μ.a μ.b + α * S := by
    ...
    · simp only [norm2, decide_eq_true_eq] at hn
      ...
      have hfin0 : ((Kx B : ℚ) : ℝ) * (P0 - 4) ≤
          ((Wlo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * ((sLoR B t0 : ℚ) : ℝ) := by
        rw [hP0]; exact_mod_cast qfin0
      have hfin1 : ((Kx B : ℚ) : ℝ) * (Q1 - 4) ≤
          ((Wlo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * ((sHiR B t0 : ℚ) : ℝ) := by
      ...
      have hfin : ((Kx B : ℚ) : ℝ) * (p - 4) ≤ ((Wlo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * S := by
        ...
        have e2 : ((Wlo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * S =
            (1 - t) * (((Wlo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * ((sLoR B t0 : ℚ) : ℝ)) +
              t * (((Wlo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * ((sHiR B t0 : ℚ) : ℝ)) := by
          rw [hSe]; ring
        rw [e1, e2]; exact hm
    ...
    · simp only [mean2, Bool.and_eq_true, decide_eq_true_eq] at hm
      ...
      have hmean : ((Dm B : ℚ) : ℝ) * p ≤ ((jLo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * dd * S := by
        ...
        have e2 : ((jLo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * dd * S =
            (1 - t) * (((jLo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * dd * ((sLoR B t0 : ℚ) : ℝ)) +
              t * (((jLo B : ℚ) : ℝ) + ((betaLo B : ℚ) : ℝ) * dd * ((sHiR B t0 : ℚ) : ℝ)) := by
          rw [hSe]; ring
        rw [e1, e2]; exact hm
...
#check @CKLaneE.LSR.check_sound
#print axioms CKLaneE.LSR.check_sound
