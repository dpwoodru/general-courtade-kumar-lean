...
/-!
# Lane E: archived-leaf refinement trees with relative-floor leaves

`RT2` extends the refinement trees of `CKLaneE.S` by leaves certified with the relative-floor
log-sum certificate `CKLaneE.LSR` (entropy floor `t0·C0` used exactly).  `leafS_of_check2` is the
corresponding archived-leaf bridge; the conclusion is the same `CKLaneE.S.LeafS`.
-/
...
inductive RT2
  ...
  | node (ax : ℕ) (v : ℚ) (l r : RT2)

def RT2.check (t0 : ℚ) : RT2 → Box2 → Bool
...
theorem RT2.sound (t0 : ℚ) (ht0 : 0 ≤ t0) : ∀ (t : RT2) (R : Box2), t.check t0 R = true → GoodR t0 R
  ...
  | .node ax v l r, R, h => by
      simp only [RT2.check, Bool.and_eq_true] at h
      have hl := RT2.sound t0 ht0 l _ h.1
      have hr := RT2.sound t0 ht0 r _ h.2
      intro k μ hab h1 h2 h3 h4 hE hact
      by_cases hax : ax = 0
      · subst hax
...
def leafCheck2 (B : SBox) (r1 r2 : ℚ) (t : RT2) : Bool :=
...
theorem leafS_of_check2 (B : SBox) (r1 r2 : ℚ) (t : RT2) (h : leafCheck2 B r1 r2 t = true) :
    LeafS B := by
  simp only [leafCheck2, Bool.and_eq_true, Bool.or_eq_true, decide_eq_true_eq] at h
  obtain ⟨⟨⟨⟨hx0, hxx, ht0, hr1p, hr12, hr2⟩, hA⟩, hB⟩, ht⟩ := h
  have hG := RT2.sound B.t0 ht0 t _ ht
...
#check @CKLaneE.S.leafS_of_check2
#print axioms CKLaneE.S.leafS_of_check2
