...
/-!
# Concrete bivariate Taylor semantics for the E8 t derivative
...
theorem centeredReplay_contains {s0 t0 x y : ℝ}
    ...
    (hc : CenterEnclosed (mixed qJet s0 t0))
    (hr : ∀ u ∈ Ioo (0 : ℝ) 1,
      RemainderEnclosed (mixed qJet (s0+x*u) (t0+y*u)))
  ...
  have henclosed : centeredReplay.Contains ((deltaTJet s0 t0 x y).d0 1) := by
    apply endpoint_mem
      ...
      (c := mixed qJet s0 t0)
      (r := fun u => mixed qJet (s0+x*u) (t0+y*u))
