...
/-! Explicit high double-cap cap tail. This source is under local Lean
development; only declarations that compile and audit are accepted. -/
...
theorem doubleCapHighTailC_bounds {x : ℝ}
    (hx : 0 < x) (hx32 : x ≤ 1 / 32) :
    x ≤ doubleCapHighTailC x ∧
      doubleCapHighTailC x ≤ (51 / 50 : ℝ) * x := by
  ...
  have hCn2half : SmallMean.Cn (2 * x) / 2 ≤
    ...
    calc
      ...
      _ = x ^ 2 / (1 - 4 * x ^ 2) := by
        ...
        ring
  have hCn2small : SmallMean.Cn (2 * x) / 2 ≤ 1 / 1020 := by
    have hrat : x ^ 2 / (1 - 4 * x ^ 2) ≤ 1 / 1020 := by
      apply (div_le_iff₀ hgap).2
      nlinarith [hu]
    exact hCn2half.trans hrat
  have hLlo : (689 / 1000 : ℝ) ≤ L := by
...
#print axioms doubleCapHighTailC_bounds
