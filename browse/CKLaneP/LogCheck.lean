/-
Lane P — verified rational enclosures of `Real.log` (reflective, kernel-evaluable).

Design follows Lane C2's `CKLaneC2.LogCheck` (same Mathlib series bounds), re-implemented in
this lane's namespace so that this lane does not depend on another lane's mutable oleans.

`atSum w n = Σ_{i<n} w^(2i+1)/(2i+1)`.  For rational `0 ≤ w < 1`, Mathlib gives
    2·atSum w n ≤ log((1+w)/(1-w)) ≤ 2·atSum w n + 2 w^(2n+1)/(1-w^2).
`logChk q k n lo hi` accepts iff, with `q' = q / 2^k ≥ 1` and `w = (q'-1)/(q'+1)`,
    lo ≤ k·L2lo + 2·atSum w n   and   k·L2hi + 2·atSum w n + 2 w^(2n+1)/(1-w^2) ≤ hi,
where `[L2lo, L2hi]` is a certified enclosure of `log 2`.  Soundness: `logChk_sound`.
-/
import Mathlib
...
/-- Certified enclosure of `log 2` (from `w = 1/3`, 40 terms). -/
def L2lo : ℚ := 2 * atSum (1 / 3) 40
def L2hi : ℚ := 2 * atSum (1 / 3) 40 + 2 * ((1 / 3 : ℚ) ^ 81 / (1 - (1 / 3 : ℚ) ^ 2))

theorem log2_bounds : (L2lo : ℝ) ≤ Real.log 2 ∧ Real.log 2 ≤ (L2hi : ℝ) := by
...
def logChk (q : ℚ) (k n : ℕ) (lo hi : ℚ) : Bool :=
  decide (1 ≤ q / 2 ^ k ∧
    lo ≤ (k : ℚ) * L2lo + 2 * atSum ((q / 2 ^ k - 1) / (q / 2 ^ k + 1)) n ∧
    (k : ℚ) * L2hi + 2 * atSum ((q / 2 ^ k - 1) / (q / 2 ^ k + 1)) n +
      2 * (((q / 2 ^ k - 1) / (q / 2 ^ k + 1)) ^ (2 * n + 1) /
        (1 - ((q / 2 ^ k - 1) / (q / 2 ^ k + 1)) ^ 2)) ≤ hi)

theorem logChk_sound {q lo hi : ℚ} {k n : ℕ} (h : logChk q k n lo hi = true) :
  ...
  have hloR : (lo : ℝ) ≤ (k : ℝ) * (L2lo : ℝ) + 2 * ((atSum w n : ℚ) : ℝ) := by exact_mod_cast hlo
  have hhiR : (k : ℝ) * (L2hi : ℝ) + 2 * ((atSum w n : ℚ) : ℝ) +
      2 * ((w : ℝ) ^ (2 * n + 1) / (1 - (w : ℝ) ^ 2)) ≤ (hi : ℝ) := by exact_mod_cast hhi
  rw [hlog]
  constructor <;> linarith

/-- Positive control: a genuine enclosure of `log 3` is accepted ... -/
theorem logChk_pos_control :
    logChk 3 1 20 (10986122886 / 10000000000) (10986122887 / 10000000000) = true := by
  decide +kernel

/-- ... and a corrupted one (upper end below `log 3 = 1.0986122886681...`) is rejected. -/
theorem logChk_neg_control :
    logChk 3 1 20 (10986122886 / 10000000000) (10986122885 / 10000000000) = false := by
  decide +kernel
