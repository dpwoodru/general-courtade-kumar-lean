...
theorem wcheckA_sound (hdouble : CanonicalDoubleCapEntropyEndpoints) (c : WCell)
  ...
  have hcheck : 0 ≤ ((c.DCb : ℚ) : ℝ) + ((c.R0 : ℚ) : ℝ) + ((c.B0 : ℚ) : ℝ) -
      ((c.M : ℚ) : ℝ) * ((c.Yv : ℚ) : ℝ) ^ 2 / (2 * (H p + H q)) := by
    have h := (Rat.cast_le (K := ℝ)).mpr hfinal
    push_cast at h
    have h2 := dip_mono_of (Y := ((c.Yv : ℚ) : ℝ)) hM0R hEL0 hELR
    linarith
  exact seam_master_Aval (by norm_num) le_rfl hp hpq hqS hys0 hysS hstat hDCb hR hB hM hM0R hYv hcheck
