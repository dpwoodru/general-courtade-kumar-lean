...
/-!
# CKLaneA3W.Contact — identities linking the TM atoms to the rootV definitions
...
theorem dom_u_pos {t ρ : ℝ} (hd : Dom t ρ) : 0 < 1 / 2 - t := by
  have := hd.2.1; have : (Tq : ℝ) = 7 / 50 := by norm_num [Tq]
  linarith
...
theorem jn_w {t ρ : ℝ} (hd : Dom t ρ) :
  ...
  have hT : (Tq : ℝ) = 7 / 50 := by norm_num [Tq]
